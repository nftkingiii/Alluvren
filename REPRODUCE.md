...
- **Distributed hosting:** with one hosting node offline, the other two still approve and execute; with one member left, nothing executes; the offline node catches up when it returns.
...
3. Install Alluvren on LocalNet (about a minute). This checks the DAR checksum, distributes `alluvren-v1` 0.3.0 to all three participants through DecMan, and allocates the business parties:

   ```bash
   bash Alluvren/infra/setup-localnet.sh
   ```

   It prints the governance party, the member hosted on each node, and six parties named `alluvren-operator`, `alluvren-treasury`, `alluvren-coo`, `alluvren-compliance`, `alluvren-investor-a` and `alluvren-investor-b`. Re-running it is safe.

4. Run the demonstrations (about 10 minutes; each prints `PASS` lines and stops at the first failure):

   ```bash
   bash Alluvren/infra/demo-localnet.sh
   ```

   To run one at a time: `bash Alluvren/infra/demo-localnet.sh shared-control` (or `app`, `outage`). Add `--all` to include the earlier two-reviewer checks.
...
| --- | --- | --- | --- | --- |
| 1 | `decman-1`, http://localhost:8081 | `app-provider` | 3975 | the member party hosted on app-provider |
| 2 | `decman-2`, http://localhost:8082 | `app-user` (also hosts the Alluvren business parties) | 2975 | the member party hosted on app-user |
...
- **`alluvren-v1 0.3.0 is not vetted`** or **`Alluvren parties not found`**: run `infra/setup-localnet.sh`.
- **Splice restarting after a reboot**: if Splice started before Postgres, restart it with `docker restart splice`.
- **The outage demo was interrupted before its cleanup ran**: bring node 1 back with `docker start decman-1 decman-3`, then reconnect its participant by re-running `bash Alluvren/infra/test-outage-localnet.sh` (it restores both on exit), or run `./hackathon/down.sh` then `./hackathon/up.sh`.
